...
def speech_to_text():
    ...
    with sr.Microphone(sample_rate=16000, chunk_size=1024) as source:  # Adjusting sample rate and chunk size
        print("Listening...")
        audio = recognizer.listen(source, timeout=5)  # Set a timeout for faster response
    try:
...
while True:
    ...
    if counter == 0:
        ...
        data = fetch_product_info()  # Now fetches from the JSON file
        if data is None:
